...
public class ElementAssertEvent extends BaseEvent {
	 ...
	 * @return EventLog
	 */
	public static EventLog assertExistsInTime(By by, int seconds) {
		Wait<WebDriver> wait = new FluentWait<WebDriver>(driver)
	 ...
	 * @return EventLog
	 */
	public static EventLog assertVisibleInTime(By by, int seconds) {
		Wait<WebDriver> wait = new FluentWait<WebDriver>(driver)
				  .withTimeout(Duration.ofSeconds(seconds))
				  .pollingEvery(Duration.ofSeconds(1))
				  .ignoring(NoSuchElementException.class);
	...
	/**
	 * assert if an element's text(fuzzy matching)
	 * @param by locate By
	 * @param expectValue
	 * @return EventLog
	 */
	public static EventLog assertText(By by, String expectValue) {
		return assertText(by, expectValue, 0);
	}
	
	/**
	 * assert if an element's text(exact matching)
	 * @param by locate By
	 * @param expectValue
	 * @return EventLog
	 */
	public static EventLog assertTextExact(By by, String expectValue) {
		return assertText(by, expectValue, 1);
	}
	 ...
	 * @return EventLog
	 */
	private static EventLog assertText(By by, String expectValue, int matchExact) {
